   ...
   // в нього інформацію з массива, для вивода інформації можете використати  готові класи підключивши bootstrap
    ...
    for (i = 0; i < array.length; i++){
       ...
       // div.innerHTML = `${array[i].name}`
        ...
        hCardTitle.innerHTML = array[i].name
        divBodyCard.appendChild(hCardTitle)
        
        let pCardText = document.createElement('p')
        pCardText.classList.add('card-text')
        pCardText.innerHTML = `phone: ${array[i].phone}`
        divBodyCard.appendChild(pCardText)
        
        let linkCard = document.createElement('a')
        linkCard.classList.add('btn')
        linkCard.classList.add('btn-primary')
        linkCard.setAttribute('href','#')
        linkCard.innerHTML = array[i].email
        divBodyCard.appendChild(linkCard)
